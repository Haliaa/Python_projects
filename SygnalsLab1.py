...
def integrateCoefficient(coefficient, k):
    return integrate.quad(coefficient, beginOfInterval, endOfInterval, args=k, limit=10000)[0]


def calculateHarmonicFrequencies(x, k):
    return integrateCoefficient(ak, k) * np.cos((k * x)) + \
        integrateCoefficient(bk, k) * np.sin((k * x))


def calculateApproximate(x, number):
    return integrateCoefficient(ak, 0) / 2 + sum(calculateHarmonicFrequencies(x, k) for k in range(1, number + 1))
...
def show_fourier_coefficients(file_object):
    ...
    for i in range(0, N + 1):
        ...
        if i == 0:
            td.append("-")
            td.append((round(integrateCoefficient(ak, i), 7)))
            continue
        td.append((str(round(integrateCoefficient(bk, i), 7))))
        td.append((str(round(integrateCoefficient(ak, i), 7))))
    columns = len(th)
...
def show_graphs():
    ...
    plt.grid(True)
    x_3 = np.linspace(beginOfInterval + 0.01, endOfInterval - 0.01, num=3000)
    plt.xlim([beginOfInterval, endOfInterval])
...
file_object = open("./SygnalsLab1.txt", "w", encoding="utf-8")
printToFile("Дана функція: 25 * sin(25 * pi * x), на проміжку х є [{}; {}]".format(beginOfInterval, endOfInterval), file_object)
show_graphs()
