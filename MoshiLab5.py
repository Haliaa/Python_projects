...
def adjustment(schedule, num_lessons, max_lessons_per_day):
    adjustment_score = max_adjustment

    for class_schedule in schedule:

        # рахуємо кількості уроків на день
        for day in class_schedule:
            num_lessons_per_day = len(day)
            if num_lessons_per_day > max_lessons_per_day:
                adjustment_score -= 10 * (num_lessons_per_day - max_lessons_per_day)

        # Перевіряємо чи вчитель відповідає предмету
        for day in class_schedule:
            for lesson in day:
        ...
        # Перевіряю чи класний керівник веде хоча б половину предметів
        ...
        # Перевіряю чи є вікно у розкладі
        window_errors = 0
        for day in class_schedule:
        ...
        for another_lesson_index in range(lesson_index + 1, len(all_lessons)):
            ...
            if first_lesson is not None and second_lesson is not None:
                ...
                if (first_lesson.day_number == second_lesson.day_number) and \
                    ...
                    same_room_errors += 1
    adjustment_score -= 10 * same_teacher_errors
    adjustment_score -= 10 * same_room_errors
...
def genetical_algorithm(lesson_names, teachers, classes, num_lessons, num_classes,
    ...
    best_schedule = copy.deepcopy(max(population, key=lambda x: adjustment(x, num_lessons, max_lessons_per_day)))
    best_adjustment = adjustment(best_schedule, num_lessons, max_lessons_per_day)

    for generation in range(max_generations):
        adjustment_scores = [adjustment(schedule, num_lessons, max_lessons_per_day) ** 5 for schedule in population]
        selected_schedules = []

        for one_population in population:

            current_adjustment = adjustment(one_population, num_lessons, max_lessons_per_day)
            if current_adjustment == max_adjustment:
                return copy.deepcopy(one_population)
            elif adjustment(one_population, num_lessons, max_lessons_per_day) > best_adjustment:
                best_schedule = copy.deepcopy(one_population)
                best_adjustment = adjustment(best_schedule, num_lessons, max_lessons_per_day)

            # Турнірна вибірка
            new_children = random.choices(population, weights=adjustment_scores, k=10)
            child = copy.deepcopy(max(new_children, key=lambda x: adjustment(x, num_lessons, max_lessons_per_day)))
    ...
    result_schedules = max(population, key=lambda x: adjustment(x, num_lessons, max_lessons_per_day))
    if adjustment(result_schedules, num_lessons, max_lessons_per_day) > best_adjustment:
...
def main():
    ...
    best_schedule = genetical_algorithm(lesson_names, teachers, classes, num_lessons, num_classes,
                                      num_days, max_lessons_per_day, room_names)


    print_schedule(best_schedule, num_days, max_lessons_per_day, num_classes, class_names)
    print(f"Пристосованість розкладу {adjustment(best_schedule, num_lessons, max_lessons_per_day)} з {max_adjustment} можливих")
